...
int main(int argc, char **argv)
{
 ...
 worldX = 0;
 worldY = 0;
  List.push_back(*(new GraphObject(windW/2, windH-300, 32, 32, true,&texture[1])));
  int n, x, y, w, h;
  ...
  for (int i = 0; i < n; i++) {
    ...
    fin >> w;
    fin >> h;
    List.push_back(*(new GraphObject(x, y + c, w, h, false,&texture[0])));
  }

  fin.close(); // закрываем файл 

  List.push_back(*(new GraphObject(677, c+4, 32, 32, false,&texture[2])));
